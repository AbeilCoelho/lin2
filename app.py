...
def index():
    ...
    return render_template("upload.html", projects=projects, active_project=session.get("active_project"))

@app.route("/set_project", methods=["POST"])
def set_project():
    project_name = re.sub(r"[^a-zA-Z0-9_\- ]", "", request.form.get("project_name", "").strip())
    if project_name:
...
def dashboard():
    ...
    cdes, metrics = [], {"total_cdes": len(grouped), "completed_cdes": 0, "total_instances": len(df), "completed_instances": 0, "needs_review": 0, "processing": 0, "auto_resolvable": 0}

    for cde_name, group in grouped:
        target_apps = set()
        for apps in group["Target App Codes"]:
            target_str = str(apps).strip()
            if target_str.lower() not in ["nan", ""]: target_apps.update([app.strip().upper() for app in target_str.split(",") if app.strip()])
...
def edit_trace():
    ...
    key = f"{data['cde_name']}|{data['app']}|{data['table']}|{data['column']}"
    cde_path = os.path.join(get_project_dir(project, "uploads"), "reporting_layers.xlsx")
    ...
    with file_lock:
        ...
        for i in range(len(stack)):
            ...
            data_row.update({
                ...
                "CDE name": cde_name, "Destination to (App Code)": dest, "Current app code": curr["app"],
                "Current app name": c_name if c_name.lower() != "nan" else "", "Current table/file name": curr.get("reconciled_table", curr["original_table"]),
                "Current column/field name": curr.get("reconciled_col", curr["original_col"]), "Source app code": s_app,
                "Source app name": s_name if s_name.lower() != "nan" else "", "Source table/file name": s_table, "Source column/field name": s_col, "QA Notes": curr.get("notes", ""),
            })
            output_rows.append(data_row)

        output_file = os.path.join(get_project_dir(project_name, "outputs"), get_safe_filename(cde_name))
        df_out = pd.DataFrame(output_rows)
        if os.path.exists(output_file):
            df_combined = pd.concat([pd.read_excel(output_file, engine="openpyxl").fillna(""), df_out], ignore_index=True).drop_duplicates(subset=["Current app code", "Current table/file name", "Current column/field name", "Source app code"], keep="first")
            df_out = df_combined
    ...
    try:
        ...
        if finished_paths:
            ...
            def score_path(p_dict):
                ...
                path_len = len(stk)
                total_cand_score = sum(n.get("cand_score", 0) for n in stk)
...
def start_auto_resolve():
    ...
    for _, row in df.iterrows():
        c_name, c_app, c_tab, c_col = str(row.get("CDE name", "")).strip(), str(row.get("Current app code", "")).strip(), str(row.get("Current table/file name", "")).strip(), str(row.get("Current column/field name", "")).strip()
        key = f"{c_name}|{c_app}|{c_tab}|{c_col}"
        if key not in done_keys and state.get(key, {}).get("status") != "NEEDS_REVIEW":
            executor.submit(process_instance_background, project, c_name, key, row.to_dict())
...
def start_single_auto_resolve():
    ...
    with file_lock:
        ...
        raw_data = df[(df["CDE name"] == data["cde_name"]) & (df["Current app code"] == data["app"])].iloc[0].to_dict()

    executor.submit(process_instance_background, project, data["cde_name"], target_key, raw_data)
...
def view_lineage(cde_name):
    project = session.get("active_project")
    output_file = os.path.join(get_project_dir(project, "outputs"), get_safe_filename(cde_name))
...
def api_graph(cde_name):
    try:
        ...
        with file_lock: df = pd.read_excel(output_file, engine="openpyxl").fillna("")
        ...
        for _, row in df.iterrows():
            ...
            c_tab, c_col = str(row.get("Current table/file name", "")).strip(), str(row.get("Current column/field name", "")).strip()
            ...
            s_tab, s_col = str(row.get("Source table/file name", "")).strip(), str(row.get("Source column/field name", "")).strip()
            ...
            if c_app not in app_meta: app_meta[c_app] = {"code": c_app, "name": c_name, "manual": False, "transformed": False, "filtered": False, "instances": []}
            ...
            if inst_entry not in app_meta[c_app]["instances"]: app_meta[c_app]["instances"].append(inst_entry)

            c_id = f"{c_app}|{c_tab.upper()}|{c_col.upper()}"
            if c_id not in inst_nodes: inst_nodes[c_id] = {"id": c_id, "label": f"🏢 Name: {c_name}\n🔑 Code: {c_app}\n📁 Table: {c_tab}\n📌 Col: {c_col}", "shape": "box", "color": {"background": "#F3F4F6", "border": "#005DAA"}, "font": {"face": "monospace", "align": "left"}}

            if s_app and s_app.lower() != "nan":
                s_id = f"{s_app}|{s_tab.upper()}|{s_col.upper()}"
                if s_id not in inst_nodes: inst_nodes[s_id] = {"id": s_id, "label": f"🏢 Name: {s_name}\n🔑 Code: {s_app}\n📁 Table: {s_tab}\n📌 Col: {s_col}", "shape": "box", "color": {"background": "#E2E8F0", "border": "#64748B"}, "font": {"face": "monospace", "align": "left"}}
                e_id = f"{s_id}->{c_id}"
                inst_edges[e_id] = {"from": s_id, "to": c_id, "arrows": "to", "color": {"color": "#005DAA"}}

                if s_app not in app_meta: app_meta[s_app] = {"code": s_app, "name": s_name, "manual": False, "transformed": False, "filtered": False, "instances": []}
                e_app_id = f"{s_app}->{c_app}"
                if e_app_id not in app_edges: app_edges[e_app_id] = {"from": s_app, "to": c_app, "arrows": "to", "color": {"color": "#10B981"}, "width": 2}

        src_inst_ids = {e["from"] for e in inst_edges.values()}
        for n_id, n in inst_nodes.items():
            if n_id not in src_inst_ids: n["color"], n["borderWidth"] = {"background": "#E0F2FE", "border": "#0284C7"}, 3
        src_app_ids = {e["from"] for e in app_edges.values()}
        for app_key, meta in app_meta.items(): app_nodes[app_key] = {"id": app_key, "meta": meta, "is_target": app_key not in src_app_ids}
...
@app.route("/tools")
def tools():
    if not session.get("active_project"): return redirect(url_for("index"))
    return render_template("tools.html", files=[f for f in os.listdir(get_project_dir(session["active_project"], "outputs")) if f.endswith(".xlsx")])
